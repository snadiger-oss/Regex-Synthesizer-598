var temporalOrderings = []; // Array of {before: "text1", after: "text2"}
var temporalSelectionMode = false;
var firstTemporalSelection = null;
...
function enableTemporalOrdering() {
    ...
    firstTemporalSelection = null;
    
    // Change button appearance to indicate active mode
    $('#temporal-order-btn').addClass('btn-success').removeClass('btn-primary');
    $('#temporal-order-btn').text('Select First Part (Click to Cancel)');
    
    // Add click handlers to highlighted spans
    $('.must-have, .char-family').css('cursor', 'pointer').on('click.temporal', function(e) {
        e.stopPropagation();
        handleTemporalClick($(this));
    });
}
...
// Function to handle clicks on highlighted spans during temporal ordering
function handleTemporalClick(span) {
    var text = span.text().trim();
    
    if (!firstTemporalSelection) {
        // First selection
        firstTemporalSelection = text;
        span.addClass('temporal-selected');
        $('#temporal-order-btn').text('Select Second Part (Click to Cancel)');
        // alert("First part selected: '" + text + "'. Now click the part that should come AFTER it.");
    } else {
        // Second selection
        var secondSelection = text;
        
        // Check if user selected the same element
        if (firstTemporalSelection === secondSelection) {
            alert("You selected the same text twice. Please select a different highlighted part.");
            return;
        }
        
        // Add the temporal ordering
        temporalOrderings.push({
            before: firstTemporalSelection,
            after: secondSelection
        });
        
        // Display the ordering in the UI
        displayTemporalOrdering(firstTemporalSelection, secondSelection);
        
        // Reset temporal mode
        cancelTemporalOrdering();
        
        // alert("Temporal ordering added: '" + firstTemporalSelection + "' must come before '" + secondSelection + "'");
    }
}

// Function to display temporal ordering in the UI
function displayTemporalOrdering(before, after) {
    var orderingId = 'temporal-' + temporalOrderings.length;
    var orderingHtml = '<div class="temporal-ordering" id="' + orderingId + '" style="margin: 10px 0; padding: 10px; background-color: #e7f3ff; border-left: 4px solid #2196F3;">' +
        '<span class="badge badge-info">Before:</span> ' + escapeHtml(before) + 
        ' <span class="badge badge-info" style="margin-left: 10px;">After:</span> ' + escapeHtml(after) +
        ' <button class="btn btn-sm btn-danger" style="margin-left: 10px;" onclick="removeTemporalOrdering(' + (temporalOrderings.length - 1) + ')">Remove</button>' +
        '</div>';
    
    $('#temporal-orderings-container').append(orderingHtml);
}

// Function to remove a temporal ordering
function removeTemporalOrdering(index) {
    temporalOrderings.splice(index, 1);
    refreshTemporalOrderingsDisplay();
}

// Function to refresh the temporal orderings display
function refreshTemporalOrderingsDisplay() {
    $('#temporal-orderings-container').empty();
    for (var i = 0; i < temporalOrderings.length; i++) {
        var ordering = temporalOrderings[i];
        var orderingHtml = '<div class="temporal-ordering" style="margin: 10px 0; padding: 10px; background-color: #e7f3ff; border-left: 4px solid #2196F3;">' +
            '<span class="badge badge-info">Before:</span> ' + escapeHtml(ordering.before) + 
            ' <span class="badge badge-info" style="margin-left: 10px;">After:</span> ' + escapeHtml(ordering.after) +
            ' <button class="btn btn-sm btn-danger" style="margin-left: 10px;" onclick="removeTemporalOrdering(' + i + ')">Remove</button>' +
            '</div>';
        $('#temporal-orderings-container').append(orderingHtml);
    }
}

// Function to cancel temporal ordering mode
function cancelTemporalOrdering() {
    temporalSelectionMode = false;
    firstTemporalSelection = null;
    
    // Remove selection styling
    $('.temporal-selected').removeClass('temporal-selected');
    
    // Remove click handlers
    $('.must-have, .char-family').off('click.temporal').css('cursor', '');
    
    // Reset button
    $('#temporal-order-btn').removeClass('btn-success').addClass('btn-primary');
    $('#temporal-order-btn').text('Mark Temporal Order');
}
...
function synthesize() {
    ...
    rows.each(function() {
        ...
        examples += '{ "input" : "' + escapeJson(inputText) + '", ' + '"exact" : [';
        ...
        // Add temporal ordering constraints for this example
        examples += ', "ordering" : [';
        
        // Find all temporal orderings that apply to this example
        // (where both 'before' and 'after' text exist in this input)
        var applicableOrderings = [];
        for (var i = 0; i < temporalOrderings.length; i++) {
            var ordering = temporalOrderings[i];
            // Check if both parts of the ordering exist in this input text
            if (inputText.includes(ordering.before) && inputText.includes(ordering.after)) {
                applicableOrderings.push(ordering);
            }
        }
        
        // Add the applicable orderings to this example
        for (var i = 0; i < applicableOrderings.length; i++) {
            var ord = applicableOrderings[i];
            examples += '{"before": "' + escapeJson(ord.before) + '", "after": "' + escapeJson(ord.after) + '"}';
            if (i < applicableOrderings.length - 1) {
                examples += ', ';
            }
        }
    ...
    // Gather synthesized programs and their annotations if any
    var regexes = '[';
    var includes = '[';
...
function submitRepeat() {
    if (typeof generalized_chars != 'undefined' && generalized_chars != null) {
        ...
        var minVal = parseInt($('#minRepeat').val());
        var maxVal = parseInt($('#maxRepeat').val());

        // Validate input
        if (isNaN(minVal) || isNaN(maxVal) || minVal < 0 || maxVal < minVal) {
            alert("Please enter valid Min and Max values. (Min should be ≥ 0 and Max ≥ Min)");
            return;
        }

        // Store it in the same 'generalizations' structure
        generalizations[generalized_chars] = {
            type: 'repeat',
            min: minVal,
            max: maxVal
        };
...
function markLiteralAndOpenModal() {
    highlightSelection('char-family'); 
    if (generalized_chars) {
        $('span.char-family').each(function() {
            if(escapeHtml($(this).text()) === generalized_chars) {
                $(this).removeClass('char-family').addClass('must-have');
                return false; 
            }
        });
    }
