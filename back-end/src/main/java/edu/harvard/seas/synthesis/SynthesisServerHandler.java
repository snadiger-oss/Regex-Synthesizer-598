...
public class SynthesisServerHandler {
	...
	private void logTemporalOrderings(String runID, Example[] examples) {
		try {
			int totalOrderings = 0;
			List<String> orderingDetails = new ArrayList<>();
			
			for (int i = 0; i < examples.length; i++) {
				Example example = examples[i];
				if (example.ordering != null && example.ordering.length > 0) {
					totalOrderings += example.ordering.length;
					
					StringBuilder sb = new StringBuilder();
					sb.append("[Example: \"").append(example.input).append("\"] ");
					
					for (Example.TemporalOrdering ordering : example.ordering) {
						sb.append("\"").append(ordering.before).append("\" before \"")
						.append(ordering.after).append("\"; ");
					}
					
					orderingDetails.add(sb.toString());
				}
			}
			
			if (totalOrderings > 0) {
				SynthesisLogger.getSynthesisLogger().logString(
					MessageFormat.format("[Run:{0}] Number of Temporal orderings: {1}\n{2}", 
						runID, totalOrderings, String.join("\n", orderingDetails))
				);
			}
		} catch (Exception e) {
	...
	public void onMessage(String message) {
		...
		if(message.equals("Reset") || message.equals("Window closed")) {
			...
			try {
				...
				logTemporalOrderings(runID, examples);
				
				// Log example details for debugging
				for (Example ex : examples) {
					if (ex.ordering != null && ex.ordering.length > 0) {
						System.out.println("Example: " + ex.input);
						for (Example.TemporalOrdering ord : ex.ordering) {
							System.out.println("  Ordering: '" + ord.before + "' before '" + ord.after + "'");
						}
					}
				}
