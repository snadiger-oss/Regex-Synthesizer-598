...
import edu.harvard.seas.synthesis.Example.TemporalOrdering;
	...
	public String parseAnnotationToSketch(Example[] examples, Regex[] regexes) {
		...
		for(Example example : examples) {
			...
			for(String s : example.generalize) {
				...
				String gen_string = s.substring(0, bgn_char_family);
				boolean skip = false;
				for (TemporalOrdering order : example.ordering) {
			...
			for (TemporalOrdering order : example.ordering) {
				int before_minLength = -1;
				int before_maxLength = -1;
				String beforeCharFamily = "";
				int after_minLength = -1;
				int after_maxLength = -1;
				String afterCharFamily = "";
				for (String s : example.generalize) {
					int bgn_char_family = s.indexOf("@@@families=");
					int bgn_minLength = s.indexOf("min=");
					int bgn_maxLength = s.indexOf("max=");
					...
					String highlighted = s.substring(0, bgn_char_family);
					System.out.println(" highlighted: " + highlighted);
					if (order.before.equals(highlighted)) {
						// before constraint
						before_minLength = minLength;
						before_maxLength = maxLength;
						beforeCharFamily = char_family;
					} else if (order.after.equals(highlighted)) {
						// after constraint
						after_minLength = minLength;
						after_maxLength = maxLength;
						afterCharFamily = char_family;
					}
				}
				for (String s : example.exact) {
					int bgn_minLength = s.indexOf("min=");
					...
					if (bgn_maxLength == -1) {
						if (bgn_minLength == -1) {
							...
							match = s.substring(0, bgn_minLength-3);
							minLength = Integer.parseInt(s.substring(bgn_minLength + 4));
						}
					}
					else {
						if (bgn_minLength == -1) {
							match = s.substring(0, bgn_maxLength-3);
							maxLength = Integer.parseInt(s.substring(bgn_maxLength + 4));
						}
						else {
							match = s.substring(0, bgn_minLength-3);
					...
					if (order.before.equals(match)) {
						...
						before_minLength = minLength;
						before_maxLength = maxLength;
					} else if (order.after.equals(match)) {
						// after constraint
						after_minLength = minLength;
						after_maxLength = maxLength;
					}
				...
				String beforeStmnt = "";
				String before_or_stmt = "";
				if (beforeCharFamily == "" && order.before.length() > 1 && before_minLength != -1 && before_maxLength != -1) {
					char[] before_chars_non = order.before.toCharArray();
					Set<Character> before_uniq = new HashSet<>();
					for (char c : before_chars_non) before_uniq.add(c);
					char[] before_chars = new char[before_uniq.size()];
					int idx = 0;
					for (char c : before_uniq) {
						before_chars[idx++] = c;
					}
					for(char c : before_chars) {
						single_chars.add("<" + c + ">");
					}
					before_or_stmt = "or(";
					for (int i = 0; i < before_chars.length - 2; i++) {
						before_or_stmt += "<" + before_chars[i] + ">,or(";
					}
					before_or_stmt += "<" + before_chars[before_chars.length - 2] + ">,<" + before_chars[before_chars.length - 1] + ">";
					for (int i = 0; i < before_chars.length - 1; i++) {
						before_or_stmt += ")";
					}
				} else {
					...
					if (before_chars.length == 1) {
						...
						for (int i = 0; i < before_chars.length - 2; i++) {
							before_or_stmt += "<" + before_chars[i] + ">, concat(";
						}
						before_or_stmt += "<" + before_chars[before_chars.length - 2] + ">,<" + before_chars[before_chars.length - 1] + ">";
						for (int i = 0; i < before_chars.length - 1; i++) {
							before_or_stmt += ")";
				...
				String afterStmnt = "";
				String after_or_stmt = "";
				if (afterCharFamily == "" && order.after.length() > 1 && after_minLength != -1 && after_maxLength != -1) {
					char[] after_chars_non = order.after.toCharArray();
					Set<Character> after_uniq = new HashSet<>();
					for (char c : after_chars_non) after_uniq.add(c);
					char[] after_chars = new char[after_uniq.size()];
					int idx = 0;
					for (char c : after_uniq) {
						after_chars[idx++] = c;
					}
					for(char c : after_chars) {
						single_chars.add("<" + c + ">");
					}
					after_or_stmt = "or(";
					for (int i = 0; i < after_chars.length - 2; i++) {
						after_or_stmt += "<" + after_chars[i] + ">,or(";
					}
					after_or_stmt += "<" + after_chars[after_chars.length - 2] + ">,<" + after_chars[after_chars.length - 1] + ">";
					for (int i = 0; i < after_chars.length - 1; i++) {
						after_or_stmt += ")";
					}
				} else {
					...
					if (after_chars.length == 1) {
						...
						for (int i = 0; i < after_chars.length - 2; i++) {
							after_or_stmt += "<" + after_chars[i] + ">, concat(";
						}
						after_or_stmt += "<" + after_chars[after_chars.length - 2] + ">,<" + after_chars[after_chars.length - 1] + ">";
						for (int i = 0; i < after_chars.length - 1; i++) {
							after_or_stmt += ")";
				...
				if (before_minLength == -1 && before_maxLength == -1) {
					if (beforeCharFamily == "") {
						beforeStmnt = before_or_stmt;
					} else {
						beforeStmnt = "<" + beforeCharFamily + ">";
					}
				} else if (before_minLength != -1 && before_maxLength != -1) {
					// both min and max length are specified
					if (beforeCharFamily == "") {
						beforeStmnt = "repeatrange(" + before_or_stmt + ","
								+ before_minLength + "," + before_maxLength + ")";
					} else {
						beforeStmnt = "repeatrange(<" + beforeCharFamily + ">,"
								+ before_minLength + "," + before_maxLength + ")";
					}
				} else if (before_minLength != -1) {
					// only min length is specified
					if (beforeCharFamily == "") {
						beforeStmnt = "repeatatleast(" + before_or_stmt + ","
								+ before_minLength + ")";
					} else {
						beforeStmnt = "repeatatleast(<" + beforeCharFamily + ">,"
								+ before_minLength + ")";
					}
				} else if (before_maxLength != -1) {
					// only max length is specified
					if (beforeCharFamily == "") {
						beforeStmnt = "repeatrange(" + before_or_stmt + ","
								+ "0," + before_maxLength + ")";
				...
				} else if (after_minLength != -1 && after_maxLength != -1) {
					// both min and max length are specified
					if (afterCharFamily == "") {
						afterStmnt = "repeatrange(" + after_or_stmt + ","
								+ after_minLength + "," + after_maxLength + ")";
					} else {
						afterStmnt = "repeatrange(<" + afterCharFamily + ">,"
								+ after_minLength + "," + after_maxLength + ")";
					}
				} else if (after_minLength != -1) {
					// only min length is specified
					if (afterCharFamily == "") {
						afterStmnt = "repeatatleast(" + after_or_stmt + ","
								+ after_minLength + ")";
					} else {
						afterStmnt = "repeatatleast(<" + afterCharFamily + ">,"
								+ after_minLength + ")";
					}
				} else if (after_maxLength != -1) {
					// only max length is specified
					if (afterCharFamily == "") {
						afterStmnt = "repeatrange(" + after_or_stmt + ","
								+ "0," + after_maxLength + ")";
				...
				sketch_includes += ("concat(" + beforeStmnt + "," + afterStmnt + "),");
				sketch_includes += ("concat(" + beforeStmnt + ",concat(?, " + afterStmnt + ")),");
			}
		...
		for(String match_s : exact_matches) {
			...
			for (Example ex : examples) {
				for (TemporalOrdering order : ex.ordering) {
